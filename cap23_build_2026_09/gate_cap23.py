...
# Dùng chung cho cả cap1_build_2026_09 và cap23_build_2026_09:
#   python3 gate_cap1.py [thư_mục]   (mặc định: thư mục chứa chính file này)
...
for f in sorted(glob.glob(f"{HERE}/out/*.json")):
    ...
    for it in lst:
        ...
        if not it.get("grade"):
            lp = str(it.get("lop") or "")
            it["grade"] = int(re.sub(r"\D", "", lp)) if re.search(r"\d", lp) else (
                d.get("grade") if isinstance(d, dict) else None)
        items.append(it)
print(f"vào: {len(items)} bài từ {len(set(i['_file'] for i in items))} file")

want = set()
for fn in ("cap1_missing.json", "cap1_todo.json", "cap23_missing.json"):
    p = f"{HERE}/{fn}"
    if os.path.exists(p):
...
tag = os.path.basename(HERE).split("_")[0]
json.dump(gated, open(f"{HERE}/{tag}_gated.json", "w"), ensure_ascii=False, indent=1)
json.dump(flagged, open(f"{HERE}/{tag}_flagged.json", "w"), ensure_ascii=False, indent=1)

print(f"\n=== CỔNG ({os.path.basename(HERE)}) ===")
