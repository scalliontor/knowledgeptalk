...
def main():
    ...
    print("Loading un-embedded math exercises...")
    with conn.cursor() as cur:
        # Vector_id is generated by post_process_math, but we want to know what's not in db ?
        # Actually let's embed EVERYTHING since we just created the table
        cur.execute("SELECT id, vector_id, lop, bo_sach, de_bai, loi_giai FROM kb_math_exercises")
        rows = cur.fetchall()
        ...
        for i in range(0, len(rows), BATCH_SIZE):
            ...
            for row in batch:
                ...
                text = f"query: bài tập toán lớp {lop} đề bài: {de}"
