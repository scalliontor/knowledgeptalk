...
def setup_db(cur):
    cur.execute("""
        CREATE TABLE IF NOT EXISTS kb_social_exercises (
...
def extract_exercises(content):
    """
    Split Social Science content into exercises using CH/Câu and Lời giải.
    """
    parsed = []
        ...
        with open(f, 'r') as fp:
            for line in fp:
                try:
                    ...
                    for ex in exs:
                        cur.execute("""
                            INSERT INTO kb_social_exercises (extracted_content_id, lop, bo_sach, de_bai, loi_giai, co_hinh, vector_id)
