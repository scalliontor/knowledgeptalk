...
CATEGORY_URLS = {
    ...
    # (1, "CTST"): "...",  # TODO: tìm URL chính xác
    # (1, "CD"): "...",
    
    # Lớp 2
    # TODO: điền các URL category khác
...
def parse_url_metadata(url: str, title: str) -> dict:
    ...
    url_lower = url.lower()
    if "tap-lam-van" in url_lower:
        metadata["content_type"] = "tap_lam_van"
    elif "luyen-tu-va-cau" in url_lower or "luyen-tu-cau" in url_lower:
...
class LoigiaihaySpider(scrapy.Spider):
    ...
    custom_settings = {
        "DOWNLOAD_DELAY": 3,                   # 3 giây giữa các request
        "RANDOMIZE_DOWNLOAD_DELAY": True,      # +/- 50%
        "CONCURRENT_REQUESTS_PER_DOMAIN": 2,   # Tối đa 2 concurrent
        "ROBOTSTXT_OBEY": True,
        "USER_AGENT": "EducationResearchBot/1.0 (research purpose; contact@example.com)",
        "AUTOTHROTTLE_ENABLED": True,
        "AUTOTHROTTLE_START_DELAY": 2,
        "AUTOTHROTTLE_MAX_DELAY": 10,
        "AUTOTHROTTLE_TARGET_CONCURRENCY": 1.0,
        "HTTPCACHE_ENABLED": True,             # Cache HTTP để re-crawl nhanh
        "HTTPCACHE_EXPIRATION_SECS": 86400 * 7,  # Cache 7 ngày
        
        # Output
        "FEEDS": {
            "output/loigiaihay_%(time)s.jsonl": {
                "format": "jsonlines",
                "encoding": "utf-8",
                "overwrite": False,
            },
