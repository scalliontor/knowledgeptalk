...
Output: cards_v_gated.json / cards_v_flagged.json"""
...
cards = []
for f in sorted(glob.glob(f"{SP}/cards_verified/verified_g*_c*.json")):
    d = json.load(open(f))
...
json.dump(gated, open(f"{SP}/cards_v_gated.json", "w"), ensure_ascii=False, indent=1)
json.dump(flagged, open(f"{SP}/cards_v_flagged.json", "w"), ensure_ascii=False, indent=1)

print(f"=== GATE TẤT ĐỊNH: {len(cards)} thẻ raw (VERIFIED 18/34 chunk, L8-L12) ===")
